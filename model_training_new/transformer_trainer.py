...
class BrainToTextDecoder_Trainer:
    ...
    def __init__(self, args):
        ...
        self.model = TransformerDecoder(
            num_features = self.args['model']['num_features'],
            num_days = len(self.args['dataset']['sessions']),
            num_classes = self.args['dataset']['n_classes'],
            patch_size = self.args['model']['patch_size'],
            patch_stride = self.args['model']['patch_stride'],
            emb_dim = self.args['model']['emb_dim'],
            depth = self.args['model']['depth'],
            num_heads = self.args['model']['num_heads'],
            input_dropout = self.args['model']['input_network']['input_layer_dropout'],
            drop_rate = self.args['model']['drop_rate'],
            attn_drop_rate = self.args['model']['attn_drop_rate'],
            drop_path_rate = self.args['model']['drop_path_rate'],
            use_day_layer = self.args['model']['use_day_layer'],
            max_mask_pct= self.args['model']['max_mask_pct'],
            num_masks= self.args['model']['num_masks'],
            mask_token_zeros= self.args['model']['mask_token_zeros'],
        )
        ...
        for name, param in self.model.named_parameters():
            ...
            if not self.args['model']['transformer_trainable']:
                if 'patch_embed' in name or 'transformer' in name:
    ...
    def train(self):
        ...
        wandb.init(
            ...
            config={
                    ...
                    "weight_decay": self.args['weight_decay'],
                    "batch_size": self.args['dataset']['batch_size'], 
                    "grad_norm_clip_value": self.args['grad_norm_clip_value'],
                    "day_per_batch": self.args['dataset']['days_per_batch'],
                    "white_noise_std": self.args['dataset']['data_transforms']['white_noise_std'],
                    "constant_offset_std": self.args['dataset']['data_transforms']['constant_offset_std'],
                    "random_cut": self.args['dataset']['data_transforms']['random_cut'],
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            with torch.autocast(device_type = "cuda", enabled = self.args['use_amp'], dtype = torch.bfloat16):
                ...
                logits = self.model(features, day_indicies, X_len=n_time_steps)

                # print('Shapes: ',n_time_steps, adjusted_lens, phone_seq_lens, logits.shape, labels.shape)
            ...
            wandb.log({
                ...
                'train_lr': self.learning_rate_scheduler.get_last_lr()[0],
            })
